...
if (grp.action == 'add' && _wel.includes(grp.id)) {
	...
	console.log(img)
	let gifT = await GIFBufferToVideoBuffer(img)
                    const des=`
...
${metadata.desc}                     
                    `
						arus.sendMessage(grp.id,{video: gifT,gifPlayback:true,contextInfo: { mentionedJid: [num] },caption:des})
                }                     
         if (grp.action == 'remove' && _wel.includes(grp.id)) {
